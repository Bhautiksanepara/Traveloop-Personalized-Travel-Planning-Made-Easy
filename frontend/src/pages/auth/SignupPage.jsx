...
import { Lock, User, Mail, Phone, MapPin, Globe } from 'lucide-react';
        ...
        <GlassCard className="p-10 border-2 border-brand-navy/5 flex flex-col items-center bg-white shadow-[0_40px_80px_-20px_rgba(15,23,42,0.1)] !rounded-[40px]" hover={false}>
          ...
          <form className="w-full space-y-8" onSubmit={(e) => e.preventDefault()}>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {[
                { label: 'First Name', placeholder: 'Felix', icon: User },
                { label: 'Last Name', placeholder: 'Wanderer', icon: User },
                { label: 'Email', placeholder: 'felix@travel.com', icon: Mail },
                { label: 'Phone Number', placeholder: '+1 234 567 890', icon: Phone },
                { label: 'City', placeholder: 'Lisbon', icon: MapPin },
                { label: 'Country', placeholder: 'Portugal', icon: Globe },
              ].map((field, i) => (
                <motion.div 
                  key={field.label}
            ...
            <motion.div 
              ...
              className="space-y-3"
            >
              <label className="text-[11px] font-black text-brand-slate uppercase tracking-[0.3em] ml-1">Additional Information</label>
              <textarea 
                rows={3}
                placeholder="Tell us about your travel style..."
                className="w-full bg-brand-navy/[0.02] border-2 border-brand-navy/5 rounded-[20px] py-4 px-6 text-brand-navy placeholder:text-brand-slate/30 focus:outline-none focus:border-brand-sky/30 focus:bg-white focus:ring-4 focus:ring-brand-sky/5 transition-all font-bold text-sm cursor-pointer resize-none"
              />
            </motion.div>
